...
public class AuthService {
    ...
    public String getRoles(String authorizationHeader) throws NoSuchAlgorithmException, InvalidKeySpecException {
        String jwt = null;
        if (authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            jwt = authorizationHeader.substring(7);
        }
        ...
        if (jwt != null) {
            PublicKey publicKey = getPublicKey(publicKeyString);
            Claims claims = Jwts.parser().setSigningKey(publicKey).parseClaimsJws(jwt).getBody();
            String roles = claims.get("realm_access", Map.class).get("roles").toString();
            return roles;
        } else {
            return "Token JWT no válido";
        } 
